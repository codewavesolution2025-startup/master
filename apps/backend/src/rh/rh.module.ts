...
export class RhService {
  ...
  async getDashboardKpis() {
    const [effectif, couts, conges, formations, presences] = await Promise.all([
      ...
      this.db.query(`
        SELECT
          COUNT(*) as total,
          COUNT(*) FILTER (WHERE f.statut='PLANIFIEE') as planifiees,
  ...
  async getCoutMoParOf() {
    return this.db.query(`
      ...
      LEFT JOIN utilisateurs u ON u.id = dp.operateur_id
      LEFT JOIN employes e ON e.email = u.email AND e.statut='ACTIF'
